...
class Request
{
    ...
    public function validate(array $rules)
    {
            ...
            foreach ($rulesArray as $rule){
                if ($rule==='required' && !isset($data[$field])){
                  ...
                  if ($rule==='string' && isset($data[$field])&& !is_string($data[$field])){
                    $errors[$field][] = "Campo Campo debe ser un string";
                    continue;
                }
                  if ($rule==='int' && isset($data[$field]) && !is_int($data[$field]&& !is_int($data[$field]))){
                    $errors[$field][] = "Campo debe ser un numero entero";
                    continue;
                }
                  if ($rule==='numeric' && !isset($data[$field])&& !is_numeric($data[$field])){
                    $errors[$field][] = "Campo debe ser un numero";
                    continue;
                }
                if (str_starts_with($rule,'max:')){
                    ...
                    if(isset($data[$field])&& $dataLen>$max){
                        $errors[$field][] = "Campo debe ser mayor de $max";
                        continue;
                    }
                }
                ...
                if (str_starts_with($rule,'min:')){
                    ...
                    if(isset($data[$field])&& $dataLen<$min){
                        $errors[$field][] = "Campo debe ser mayor de $min";
                        continue;
                    }
                }
            }       
        ...
        if (!$errors){
            Response::json(['message'=>'Datos invalidos','errors'=>$errors],400);
            exit;
        }
    }   
}